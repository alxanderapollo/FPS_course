...
#meaning this is an automatic weapon
@export var automatic = false

@export var damage = 5
@export var ammo  = 30
#puts a small delay on the firearm
...
func set_bodies_to_exclude(bodies:Array):
	bullet_emitter.set_bodies_to_execule(bodies)

func attack(input_just_pressed: bool, input_hold: bool):
	if !automatic and !input_just_pressed: return
	if automatic and !input_hold: return
	if ammo == 0: 
		if input_just_pressed:  out_of_ammo.emit()
		return
	
	var cur_time = Time.get_ticks_msec()/1000.0
	if cur_time - last_attack_time < attack_rate: return
...
func set_active (a: bool):
	visible = a
	if !a: animation_player.play("RESET")
